...
export default function SitesGrid({ showOnlyFavorites = false }: Props) {
    ...
    const [checkoutUrl, setCheckoutUrl] = useState<string>('https://pv-meusitecomia.vercel.app/');
    ...
    const handleDeploy = async (e: FormEvent) => {
        ...
        try {
            ...
            setStatus('Repositório criado! Construindo o site na Vercel...');

            if (data.deploymentId) {
                let pollCount = 0;
                const MAX_POLLS = 45; // 45 x 4s = 3 minutos de timeout
                const pollInterval = setInterval(async () => {
                    pollCount++;
                    if (pollCount > MAX_POLLS) {
                        clearInterval(pollInterval);
                        setStatus('O deploy está demorando mais que o esperado. Verifique em "Meus Sites" daqui a alguns minutos.');
                        setLoading(false);
                        return;
                    }
                    try {
                        ...
                        if (checkData.readyState === 'READY') {
                            ...
                            setStatus(`Ocorreu um erro na compilação do seu site. Nossa equipe já foi notificada. Se precisar de ajuda, envie este código ao suporte: ${code}`);
                            setLoading(false);
                        } else {
                            ...
                            setStatus(stateMap[checkData.readyState] || 'Construindo o site...');
                        }
                    } catch {
                ...
                setStatus('Deploy iniciado! Verifique em "Meus Sites" daqui a alguns minutos.');
                setLoading(false);
            }
        } catch (err: any) {
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {(() => {
                    ...
                    return displayed.map((template, templateIndex) => {
                        const cardCategories = template.category_ids
                            ? categories.filter(c => template.category_ids.includes(c.id))
                            : [];

                        const isWalker = template.name?.toLowerCase().includes('walker');

                        return (
                            <div
                                key={template.id}
                                {...(isWalker ? { 'data-tour': 'template-walker' } : templateIndex === 0 ? { 'data-tour': 'primeiro-template' } : {})}
                            ...
                            >
                                ...
                                {template.is_locked && (
                                    <div className="absolute top-3 left-3 z-10">
                                        <span className="bg-amber-500 text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-sm flex items-center gap-1 uppercase tracking-wider">
                    ...
                    <div id="slide-over-panel" className="relative w-full max-w-[1380px] h-full bg-[#f8fafc] shadow-2xl flex flex-col overflow-y-auto pb-12 animate-slide-in-right">
                        ...
                        <div className="w-full mx-auto px-4 lg:px-8 py-4 flex flex-col">
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-y-4 lg:gap-x-8 items-stretch">
                                ...
                                <div data-tour="template-preview" className="lg:col-span-8 lg:row-start-1 bg-white rounded-3xl overflow-hidden shadow-xl border border-gray-200 flex items-center justify-center p-2 lg:p-3 relative group w-full min-h-[340px] lg:min-h-[440px] max-h-[550px]">
                                ...
                                <div className="lg:col-span-4 lg:row-start-1 bg-white rounded-3xl shadow-xl shadow-[#7c3aed]/5 border border-[#7c3aed]/10 p-5 lg:p-6 flex flex-col h-full overflow-hidden max-h-[85vh] xl:max-h-[550px]">
                                    ...
                                    <div className="mt-5 space-y-2.5">
                                        {selectedTemplate.is_locked && (
                                            <div className="bg-amber-50 text-amber-800 p-4 rounded-xl border border-amber-200 mb-4 animate-fade-in shadow-sm">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="shrink-0"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
                                                    <span className="font-bold text-sm">Este template está em desenvolvimento</span>
                                                </div>
                                                <p className="text-xs opacity-90">
                                                    {selectedTemplate.release_date
                                                        ? `Data prevista de liberação: ${new Date(selectedTemplate.release_date).toLocaleString('pt-BR', { dateStyle: 'long', timeStyle: 'short' })}`
                                                        : "Aguarde novidades! Em breve ele estará disponível para você."
                                                    }
                                                </p>
                                            </div>
                                        )}
                                        {subscriptionStatus !== 'active' && !selectedTemplate.is_locked && (
                                            <div className="bg-amber-50 text-amber-700 p-3.5 rounded-xl text-xs mb-3 border border-amber-200">
                                                <p className="font-bold mb-1">Assinatura inativa</p>
                                                <p className="text-amber-600 mb-2">Ative seu plano para criar sites ilimitados com deploy automático.</p>
                                                <a href={checkoutUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 bg-amber-600 hover:bg-amber-700 text-white px-3 py-1.5 rounded-lg text-xs font-bold transition">
                                                    Ativar Plano Agora
                                                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h6v6" /><path d="M10 14 21 3" /></svg>
                                                </a>
                                            </div>
                                        )}
                                        {subscriptionStatus !== 'active' ? (
                                            ...
                                            <button disabled className="w-full flex items-center justify-center gap-2 py-2.5 lg:py-3 px-4 border-2 border-amber-100 rounded-xl text-xs font-bold text-amber-600 bg-amber-50 cursor-not-allowed opacity-70">
                                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
                                                Preview Indisponível
                                            </button>
                                        ) : (
                                            <a
                                                data-tour="btn-previa"
            ...
            {isDeployMode && selectedTemplate && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 relative">
                        ...
                        <form onSubmit={handleDeploy} data-tour="deploy-form" className="space-y-4 text-left">
                            <div>
                            ...
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1">Senha do Painel Admin</label>
                                <p className="text-[11px] text-gray-400 mb-1.5">Protege o editor de conteúdo do seu site (acessível em seusite.com/admin)</p>
                                <input
                                    type="password"
                                    required
                                    disabled={loading}
                            ...
                            {deployedUrl ? (
                                <div className="mt-6 space-y-4">
                                    ...
                                    <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
                                        ...
                                        <h4 className="font-bold text-green-800 text-lg">Seu site está no ar!</h4>
                                        <p className="text-green-600 text-xs mt-1">{deployedUrl}</p>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noreferrer"
                                            className="w-full flex items-center gap-3 py-3 px-4 rounded-xl text-sm font-bold text-white bg-green-500 hover:bg-green-600 transition-all"
                                        >
                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M15 3h6v6"/><path d="M10 14 21 3"/><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/></svg>
                                            Ver meu site
                                        </a>
                                        <a
                                            href={`https://${deployedUrl}/admin`}
                                            target="_blank"
                                            rel="noreferrer"
                                            className="w-full flex items-center gap-3 py-3 px-4 rounded-xl text-sm font-bold text-[#7c3aed] bg-[#7c3aed]/10 hover:bg-[#7c3aed]/20 border border-[#7c3aed]/20 transition-all"
                                        >
                                        ...
                                        <a
                                            href="/meus-sites"
                                            className="w-full flex items-center gap-3 py-3 px-4 rounded-xl text-sm font-bold text-gray-600 bg-gray-50 hover:bg-gray-100 border border-gray-200 transition-all"
                                        >
                                            ...
                                            Ver todos os meus sites
                                        </a>
                                    </div>

                                    <p className="text-[10px] text-gray-400 text-center mt-2">
                                        A senha do painel admin é a que você definiu acima. Acesse em <span className="font-mono">seusite.com/admin</span>
                                    </p>
                                </div>
                            ) : (
                                <button
